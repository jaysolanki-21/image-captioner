...
const Header = ({ onOpenModal, isAuthenticated, setIsAuthenticated }) => {
  ...
  // Local mirror of auth state to avoid transient UI mismatch while parent updates
  const [localAuth, setLocalAuth] = useState(isAuthenticated);

  // Keep localAuth in sync when parent prop changes
  useEffect(() => {
    setLocalAuth(isAuthenticated);
  }, [isAuthenticated]);

  // On mount, validate auth with server to correct any stale UI state
  useEffect(() => {
    let mounted = true;
    const check = async () => {
      try {
